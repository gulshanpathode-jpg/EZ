...
//   - result_id ties a later POST /api/inspections/feedback back to this run.
...
//
// Feedback: POST /api/inspections/feedback
...
app.post("/api/inspections/verify", upload.any(), async (req, res) => {
  let payload;
  try {
...
app.post("/api/inspections/feedback", (req, res) => {
  const body = req.body || {};
...
app.listen(PORT, () => {
  console.log(`AI Verifier backend listening on http://localhost:${PORT}`);
  console.log(`Endpoint: POST http://localhost:${PORT}/api/inspections/verify`);
});
